...
interface RedstoneClientAdapter {
    ...
    /**
     * Frequently invoked tick function, transferring cached RAM data from and to the adapter
     * application on the system side.
     *
     * - The method shall manage reading and writing to this channel, but also opening, closing, and
     ...
     *
     * - If the outputs are uninitialized or marked as changed, their value shall be written and the
     *   change-marker unset.
     */
    fun tick()

    /**
     * Returns true if the used IPC channels are successfully opened at adapter side of this
     * application. This may not guarantee that a bidirectional communication is possible (depending
     * on the IPC method used), but indicate to the tick() method to close-retry-open, as well as
     * indicate a "issue" marker to the user. Note: A serial port, or a pipe/fifo, or a shared
     * memory, or UDP socket may be opened successfully, but is not possible to determine if someone
     * is on the other side.
     */
    fun isOpen(): Boolean

    /**
     * Returns the RAM stored/cached input data word, holding 16 4bit channels.
     *
    ...
    /**
     * Sets the RAM stored/cached input data word, and marks the input word as changed.
     *
    ...
    /**
     * Returns true if the input data word is marked as changed.
     *
    ...
    /**
     * Returns the RAM stored/cached output data word, holding 16 4bit channels.
     *
    ...
    /**
     * Sets the RAM stored/cached output data word, and marks the input word as changed.
     *
    ...
    /**
     * Returns true if the output data word is marked as changed.
     *
    ...
    fun setInputChannel(channel: Int, value: Int) =
        setInputs(
            (getInputs() and (0xfL.inv() shl (4 * channel))) or
                ((value and 0xf).toLong() shl (4 * channel))
        )
    ...
    fun setOutputChannel(channel: Int, value: Int) =
        setOutputs(
            (getOutputs() and (0xfL.inv() shl (4 * channel))) or
                ((value and 0xf).toLong() shl (4 * channel))
        )
}
