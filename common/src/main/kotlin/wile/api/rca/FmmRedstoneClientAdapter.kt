...
class FmmRedstoneClientAdapter {
    ...
    /**
     * Memory Mapping auxiliary class. Wraps the common processes of using
     * MappedByteBuffer/RandomAccessFile in java. Under Linux the native `mmap()` functionality will
     * be implicitly used, Windows provides the similar functionality via the `CreateFileMapping()`/
     * `OpenFileMapping()` WINAPI functions (a bit more is needed, but in essence these are the key
     * functions).
     */
    private class FileMemMap(
        ...
        fun set(index: Int, value: Byte) {
            if (index in 0 until size()) mapData[index] = value
        }
        ...
        fun open(initialize: Boolean): Boolean {
            ...
            return try {
                ...
                val mode =
                    if (isWrite) FileChannel.MapMode.READ_WRITE else FileChannel.MapMode.READ_ONLY
    ...
    class Adapter(private val isSystemSide: Boolean) : RedstoneClientAdapter {
        ...
        /** @see RedstoneClientAdapter */
        override fun setInputs(value: Long) {
            if (value != inputDataWord) {
                inputDataWord = value
        ...
        /** @see RedstoneClientAdapter */
        override fun setInputsChanged(changed: Boolean) {
        ...
        /** @see RedstoneClientAdapter */
        override fun setOutputs(value: Long) {
            if (value != outputDataWord) {
        ...
        /** @see RedstoneClientAdapter */
        override fun setOutputsChanged(changed: Boolean) {
        ...
        companion object {
            ...
            private fun hex2nibble(b: Byte): Int =
                when {
            ...
            private fun nibble2hex(n: Int): Byte =
                when {
                    n <= 0x0 -> '0'.code.toByte()
            ...
            /**
             * Returns the IPC map file path ("[prefix].i.mmap" or "[prefix].o.mmap") from the
             * reference perspective of the MC mod side.
             *
             * @param mcSideOutput True if output for the mod side and input for system side, or
             *   vice versa.
             * @return System formatted IPC file path string representation.
             */
            @JvmStatic
            fun ipcIoPath(mcSideOutput: Boolean): String =
                Auxiliaries.getGameDirectory()
                    .resolve("redstonepen." + (if (mcSideOutput) 'o' else 'i') + ".mmap")
                    .toString()

            /**
             * Returns true if the feature is enabled. Initialized after the first invocation of
             * `instance()`.
             *
             * @return bool
             */
            @JvmStatic fun available(): Boolean = featureEnabled && singletonInstance != null

            /**
             * Mod side singleton instance getter. Returns `null` on error or if the preconditions
             * to allow enabling this feature are not met.
             *
             * If you use this class for writing a system-side adapter program, call the constructor
             * directly.
             *
             * @return Adapter
             */
            ...
            fun instance(): Adapter? {
                ...
                return try {
                    if (
                        !Files.exists(Path.of(ipcIoPath(false))) ||
