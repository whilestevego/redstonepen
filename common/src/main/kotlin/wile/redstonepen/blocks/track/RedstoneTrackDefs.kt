...
object RedstoneTrackDefs {
    ...
    @JvmField val REDSTONE_UPDATE_DIRECTIONS: Array<Direction> = arrayOf(
        Direction.WEST, Direction.EAST, Direction.DOWN, Direction.UP, Direction.NORTH, Direction.SOUTH
    )

    object connections {
        @JvmField val CONNECTION_BIT_ORDER: Array<Direction> = arrayOf(
            Direction.DOWN, Direction.UP, Direction.NORTH, Direction.SOUTH, Direction.EAST, Direction.WEST
        )

        @JvmField val CONNECTION_BIT_ORDER_REV: Map<Direction, Int> = mapOf(
            Direction.DOWN to 0,
            Direction.UP to 1,
        ...
        @JvmField val BULK_FACE_MAPPING: Map<Long, Direction> = mapOf(
        ...
        @JvmField val BULK_FACE_MAPPING_REV: Map<Direction, Long> = mapOf(
        ...
        @JvmField val WIRE_FACE_DIRECTION_MAPPING: Map<Long, Tuple<Direction, Direction>> = mapOf(
        ...
        @JvmField val INTERNAL_EDGE_CONNECTION_MAPPING: Map<Long, Tuple<Direction, Direction>> = mapOf(
        ...
        @JvmStatic fun getWireBit(face: Direction, wire_direction: Direction): Long =
            WIRE_FACE_DIRECTION_MAPPING.entries
                .firstOrNull { it.value.getA() == face && it.value.getB() == wire_direction }
                ?.key ?: 0L

        @JvmStatic fun getWireBitSideAndDirection(wirebit: Long): Tuple<Direction, Direction> =
            WIRE_FACE_DIRECTION_MAPPING.getOrDefault(wirebit, Tuple(Direction.DOWN, Direction.DOWN))

        @JvmStatic fun getVanillaWireConnectionDirections(mask: Long): List<Direction> {
        ...
        @JvmStatic fun hasVanillaWireConnection(mask: Long, side: Direction): Boolean = when (side) {
        ...
        @JvmStatic fun hasBulkConnection(mask: Long, side: Direction): Boolean =
            (BULK_FACE_MAPPING_REV[side]!! and mask) != 0L

        @JvmStatic fun hasRedstoneConnection(mask: Long, side: Direction): Boolean = when (side) {
        ...
        @JvmStatic fun getWireElementsOnFace(face: Direction): Long {
            val index = CONNECTION_BIT_ORDER_REV[face] ?: 0
            return 0xfL shl ((index * 4) + STATE_FLAG_WIR_POS)
        }

        @JvmStatic fun getAllElementsOnFace(face: Direction): Long {
            val index = CONNECTION_BIT_ORDER_REV[face] ?: 0
            return (0xfL shl ((index * 4) + STATE_FLAG_WIR_POS)) or (0x1L shl (index + STATE_FLAG_CON_POS))
        }
    }
    ...
    object shape {
        ...
        private val DOWN_SHAPE: VoxelShape = Auxiliaries.getUnionShape(
            Auxiliaries.getPixeledAABB(8 - SHAPE_TRACK_HALFWIDTH, 0.0, 0.0, 8 + SHAPE_TRACK_HALFWIDTH, SHAPE_LAYER_THICKNESS, 16.0),
            Auxiliaries.getPixeledAABB(0.0, 0.0, 8 - SHAPE_TRACK_HALFWIDTH, 16.0, SHAPE_LAYER_THICKNESS, 8 + SHAPE_TRACK_HALFWIDTH)
        )
        private val UP_SHAPE: VoxelShape = Auxiliaries.getUnionShape(
            Auxiliaries.getPixeledAABB(8 - SHAPE_TRACK_HALFWIDTH, 16 - SHAPE_LAYER_THICKNESS, 0.0, 8 + SHAPE_TRACK_HALFWIDTH, 16.0, 16.0),
            Auxiliaries.getPixeledAABB(0.0, 16 - SHAPE_LAYER_THICKNESS, 8 - SHAPE_TRACK_HALFWIDTH, 16.0, 16.0, 8 + SHAPE_TRACK_HALFWIDTH)
        )
        private val WEST_SHAPE: VoxelShape = Auxiliaries.getUnionShape(
            Auxiliaries.getPixeledAABB(0.0, 0.0, 8 - SHAPE_TRACK_HALFWIDTH, SHAPE_LAYER_THICKNESS, 16.0, 8 + SHAPE_TRACK_HALFWIDTH),
            Auxiliaries.getPixeledAABB(0.0, 8 - SHAPE_TRACK_HALFWIDTH, 0.0, SHAPE_LAYER_THICKNESS, 8 + SHAPE_TRACK_HALFWIDTH, 16.0)
        )
        private val EAST_SHAPE: VoxelShape = Auxiliaries.getUnionShape(
            Auxiliaries.getPixeledAABB(16 - SHAPE_LAYER_THICKNESS, 0.0, 8 - SHAPE_TRACK_HALFWIDTH, 16.0, 16.0, 8 + SHAPE_TRACK_HALFWIDTH),
            Auxiliaries.getPixeledAABB(16 - SHAPE_LAYER_THICKNESS, 8 - SHAPE_TRACK_HALFWIDTH, 0.0, 16.0, 8 + SHAPE_TRACK_HALFWIDTH, 16.0)
        )
        private val NORTH_SHAPE: VoxelShape = Auxiliaries.getUnionShape(
            Auxiliaries.getPixeledAABB(0.0, 8 - SHAPE_TRACK_HALFWIDTH, 0.0, 16.0, 8 + SHAPE_TRACK_HALFWIDTH, SHAPE_LAYER_THICKNESS),
            Auxiliaries.getPixeledAABB(8 - SHAPE_TRACK_HALFWIDTH, 0.0, 0.0, 8 + SHAPE_TRACK_HALFWIDTH, 16.0, SHAPE_LAYER_THICKNESS)
        )
        private val SOUTH_SHAPE: VoxelShape = Auxiliaries.getUnionShape(
            Auxiliaries.getPixeledAABB(0.0, 8 - SHAPE_TRACK_HALFWIDTH, 16 - SHAPE_LAYER_THICKNESS, 16.0, 8 + SHAPE_TRACK_HALFWIDTH, 16.0),
            Auxiliaries.getPixeledAABB(8 - SHAPE_TRACK_HALFWIDTH, 0.0, 16 - SHAPE_LAYER_THICKNESS, 8 + SHAPE_TRACK_HALFWIDTH, 16.0, 16.0)
        )

        private val shape_cache: Array<VoxelShape?> = arrayOfNulls(64)

        @JvmStatic fun get(faces: Int): VoxelShape {
    ...
    object models {
        @JvmField val STATE_WIRE_MAPPING: Map<Long, String> = mapOf(
            0x00000000L to "none",
            0x00000001L to "dn",  0x00000002L to "ds",  0x00000004L to "de",  0x00000008L to "dw",
            0x00000010L to "un",  0x00000020L to "us",  0x00000040L to "ue",  0x00000080L to "uw",
            0x00000100L to "nu",  0x00000200L to "nd",  0x00000400L to "ne",  0x00000800L to "nw",
            0x00001000L to "su",  0x00002000L to "sd",  0x00004000L to "se",  0x00008000L to "sw",
            0x00010000L to "eu",  0x00020000L to "ed",  0x00040000L to "en",  0x00080000L to "es",
            0x00100000L to "wu",  0x00200000L to "wd",  0x00400000L to "wn",  0x00800000L to "ws"
        )

        @JvmField val STATE_CONNECT_MAPPING: Map<Long, String> = mapOf(
            0x0000000000000000L to "none",
            0x0000000001000000L to "dc", 0x0000000002000000L to "uc",
            0x0000000004000000L to "nc", 0x0000000008000000L to "sc",
            0x0000000010000000L to "ec", 0x0000000020000000L to "wc"
        )

        @JvmField val STATE_CNTWIRE_MAPPING: Map<Long, String> = mapOf(
            0x0000000000000000L to "none",
            0x0000000001000000L to "dm", 0x0000000002000000L to "um",
            0x0000000004000000L to "nm", 0x0000000008000000L to "sm",
            0x0000000010000000L to "em", 0x0000000020000000L to "wm"
        )
    }
}
