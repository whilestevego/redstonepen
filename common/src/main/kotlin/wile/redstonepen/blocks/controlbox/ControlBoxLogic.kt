...
internal class ControlBoxLogic {
    ...
    fun errors(): Map<Int, String> =
        expressions_.invalid_entries.stream().collect(Collectors.toMap({ e -> e.offset + e.parsed.pe }, { e -> e.parsed.error }))

    fun symbol(key: String, value: Int) { symbols_[key.lowercase()] = value }

    fun symbol(key: String): Int = symbols_.getOrDefault(key.lowercase(), 0)

    fun symbols(): MutableMap<String, Int> = symbols_

    fun clearSymbols() { symbols_.clear() }

    fun code(): String = code_

    fun code(new_code: String): Boolean {
        if (code_ == new_code && !expressions_.isEmpty()) return expressions_.invalid_entries.isEmpty()
        code_ = new_code
        ...
        for (i in 0 until Defs.PORT_NAMES.size) {
            ...
            } else if (MultiLineMathExpr.VALID_SYMBOL_SUFFIXES.any { s -> expressions_.symbols.contains(port + s) }) {
    ...
    fun tick() {
        for (i in 0 until Defs.PORT_NAMES.size) {
            if ((input_mask and (0xf shl (4 * i))) != 0) symbol(Defs.PORT_NAMES[i], (input_data shr (4 * i)) and 0xf)
        }
        if (rca_input_mask != 0L) {
            for (i in 0 until 16) {
                if ((rca_input_mask and (0xfL shl (4 * i))) != 0L) symbol("di$i", ((rca_input_data shr (4 * i)) and 0xfL).toInt())
            }
        }
        expressions_.symbols.forEach { esym ->
        ...
        intr_fedges = 0
        symbol(".deadline", 40)
        val assigned = expressions_.recalculate(symbols_) { entry, _ ->
            when (entry.parsed.assignment_symbol) {
                "r", "b", "y", "g", "u", "d" -> Math.max(0, Math.min(15, entry.last_result))
                else -> entry.last_result
            }
        }
        assigned.forEach { (k, v) -> symbol(k, v) }
        output_data = 0
        for (i in 0 until Defs.PORT_NAMES.size) output_data = output_data or ((symbol(Defs.PORT_NAMES[i]) and 0xf) shl (4 * i))
        output_data = output_data and output_mask
        if (rca_output_mask != 0L) {
            rca_output_data = 0L
            for (i in 0 until 16) {
                if ((rca_output_mask and (0xfL shl (4 * i))) != 0L) {
                    rca_output_data = rca_output_data or (Math.min(15, Math.max(0, symbol("do$i"))).toLong() shl (4 * i))
                }
            }
        }
        rca_output_data = rca_output_data and rca_output_mask
    ...
    companion object {
        private fun counter_function(sym: String, x: Array<MathExpr.Expr>, m: MutableMap<String, Int>): Int {
            val nargs = x.size
            if (nargs <= 0) return 0
            ...
            q = if (nargs >= 4) Mth.clamp(q, x[2].calc(m), x[3].calc(m))
                else if (nargs >= 3) Mth.clamp(q, 0, x[2].calc(m))
                else Mth.clamp(q, 0, 0x7fffffff)
            m[sym] = q
            return q
        }

        private fun timer_on_function(sym: String, x: Array<MathExpr.Expr>, m: MutableMap<String, Int>): Int {
            if (x.size != 2) { m.remove(".$sym.clk"); m.remove("$sym.et"); m.remove("$sym.pt"); return 0 }
            val inp = x[0].calc(m); val pt = x[1].calc(m)
            if (inp <= 0) {
                m.remove(".$sym.clk"); m["$sym.et"] = 0; return MathExpr.Expr.bool_false()
            } else if (pt <= 0) {
                return MathExpr.Expr.bool_true()
            } else {
                val now = m.getOrDefault(".clock", 0)
                var et = m.getOrDefault("$sym.et", 0)
                if (et >= pt) return MathExpr.Expr.bool_true()
                else if (et <= 0) {
                    m[".$sym.clk"] = now; m["$sym.et"] = 1; m["$sym.pt"] = pt
                    m[".deadline"] = Math.min(m.getOrDefault(".deadline", 20), pt)
                    return MathExpr.Expr.bool_false()
                } else {
                    et = Math.min(now - m.getOrDefault(".$sym.clk", now), pt); m["$sym.et"] = et
                    if (et >= pt) { m.remove(".$sym.clk"); return MathExpr.Expr.bool_true() }
                    else { m[".deadline"] = Math.min(m.getOrDefault(".deadline", 20), pt - et); return MathExpr.Expr.bool_false() }
                }
            }
        }

        private fun timer_off_function(sym: String, x: Array<MathExpr.Expr>, m: MutableMap<String, Int>): Int {
            if (x.size != 2) { m.remove(".$sym.clk"); m.remove("$sym.et"); m.remove("$sym.pt"); return 0 }
            val inp = x[0].calc(m); val pt = x[1].calc(m)
            if (inp > 0) {
                m.remove(".$sym.clk"); m["$sym.et"] = 0; return MathExpr.Expr.bool_true()
            } else if (pt <= 0) {
                return MathExpr.Expr.bool_true()
            } else {
                val now = m.getOrDefault(".clock", 0)
                var et = m.getOrDefault("$sym.et", 0)
                if (et >= pt) return MathExpr.Expr.bool_false()
                else if (et <= 0) {
                    m[".$sym.clk"] = now; m["$sym.et"] = 1; m["$sym.pt"] = pt
                    m[".deadline"] = Math.min(m.getOrDefault(".deadline", 20), pt)
                    return MathExpr.Expr.bool_true()
                    ...
                    et = Math.min(now - m.getOrDefault(".$sym.clk", now), pt); m["$sym.et"] = et
                    if (et >= pt) { m.remove(".$sym.clk"); return MathExpr.Expr.bool_false() }
                    else { m[".deadline"] = Math.min(m.getOrDefault(".deadline", 20), pt - et); return MathExpr.Expr.bool_true() }
                }
            }
        }

        private fun timer_pulse_function(sym: String, x: Array<MathExpr.Expr>, m: MutableMap<String, Int>): Int {
            if (x.size != 2) { m.remove(".$sym.clk"); m.remove("$sym.et"); m.remove("$sym.pt"); return 0 }
            val inp = x[0].calc(m); val pt = x[1].calc(m)
            if (pt <= 0) return if (inp > 0) MathExpr.Expr.bool_true() else MathExpr.Expr.bool_false()
            var et = m.getOrDefault("$sym.et", 0)
            if (et > 0) {
                if (et >= pt) { if (inp <= 0) m["$sym.et"] = 0; return MathExpr.Expr.bool_false() }
                else {
                    val now = m.getOrDefault(".clock", 0)
                    et = Math.min(now - m.getOrDefault(".$sym.clk", now), pt); m["$sym.et"] = et
                    if (et >= pt) { m.remove(".$sym.clk"); return MathExpr.Expr.bool_false() }
                ...
                m[".$sym.clk"] = m.getOrDefault(".clock", 0); m["$sym.et"] = 1; m["$sym.pt"] = pt
                m[".deadline"] = Math.min(m.getOrDefault(".deadline", 20), pt)
                return MathExpr.Expr.bool_true()
            } else {
                return MathExpr.Expr.bool_false()
        ...
        private fun timer_interval_function(sym: String, x: Array<MathExpr.Expr>, m: MutableMap<String, Int>): Int {
            if (x.size < 1 || x.size > 2) { m.remove("$sym.clk"); return 0 }
            val en = if (x.size < 2) 15 else x[1].calc(m)
            if (en <= 0) { m.remove("$sym.clk"); return 0 }
            val pt = x[0].calc(m)
            if (pt <= 2) return MathExpr.Expr.bool_false()
            val now = m.getOrDefault(".clock", 0)
            val clk = m.getOrDefault("$sym.clk", now - pt)
            if (Math.abs(now - clk) >= pt) {
                m["$sym.clk"] = now; m[".deadline"] = 1; return MathExpr.Expr.bool_true()
            } else {
                m[".deadline"] = Math.min(m.getOrDefault(".deadline", 20), clk - now + pt); return MathExpr.Expr.bool_false()
            }
        }

        private fun makeFunctions(): List<MathExpr.ExprFuncDef> = listOf(
            MathExpr.ExprFuncDef("inv",   1,  { x, m -> Math.min(15, Math.max(0, 15 - x[0].calc(m))) }),
            MathExpr.ExprFuncDef("max",   -1, { x, m -> x.maxOfOrNull { it.calc(m) } ?: 0 }),
            MathExpr.ExprFuncDef("min",   -1, { x, m -> x.minOfOrNull { it.calc(m) } ?: 0 }),
            MathExpr.ExprFuncDef("lim",   -1, { x, m -> when (x.size) { 0 -> 0; 1 -> Math.min(15, Math.max(0, x[0].calc(m))); 2 -> Math.min(x[1].calc(m), Math.max(0, x[0].calc(m))); else -> Math.min(x[2].calc(m), Math.max(x[1].calc(m), x[0].calc(m))) } }),
            MathExpr.ExprFuncDef("if",    -1, { x, m -> when (x.size) { 0 -> 0; 1 -> if (x[0].calc(m) > 0) 15 else 0; 2 -> if (x[0].calc(m) > 0) x[1].calc(m) else 0; else -> if (x[0].calc(m) > 0) x[1].calc(m) else x[2].calc(m) } }),
            MathExpr.ExprFuncDef("mean",  -1, { x, m -> if (x.isEmpty()) 0 else x.sumOf { it.calc(m) } / x.size }),
            ...
            MathExpr.ExprFuncDef("tiv1",  -1, { x, m -> timer_interval_function(".tiv1", x, m) }),
            MathExpr.ExprFuncDef("tiv2",  -1, { x, m -> timer_interval_function(".tiv2", x, m) }),
            MathExpr.ExprFuncDef("tiv3",  -1, { x, m -> timer_interval_function(".tiv3", x, m) }),
    ...
    class MultiLineMathExpr {
        ...
        companion object {
            ...
            @JvmField val VALID_SYMBOL_SUFFIXES = arrayOf("", ".re", ".fe", ".co", ".co.re", ".co.fe", ".pt", ".et")

            @JvmStatic fun of(code: String): MultiLineMathExpr = of(code, "", emptyList())

            @JvmStatic fun of(code: String, assignment_variable: String): MultiLineMathExpr = of(code, assignment_variable, emptyList())

            @JvmStatic fun of(code: String, assignment_variable: String, functions: Collection<MathExpr.ExprFuncDef>): MultiLineMathExpr {
                ...
                for (line in lines) {
                    if (line.trim().isNotEmpty()) {
                        val entry = Entry(line_index, offset, MathExpr.ParsedLine.of(line, assignment_variable, functions))
                        if (entry.parsed.error.isNotEmpty()) {
                            parse_errors.add(entry)
                        } else {
                            val invalid_symbol = entry.parsed.symbols.firstOrNull { sym ->
                                val pos = sym.indexOf('.')
                                if (pos < 0) false
                                else if (pos >= sym.length - 1) true
                                else !VALID_SYMBOL_SUFFIXES.contains(sym.substring(pos))
                            }
                ...
                return if (entries.isEmpty() && parse_errors.isEmpty()) EMPTY else MultiLineMathExpr(entries, parse_errors, symbols, assignments)
            }
        }
        ...
        constructor(lines: List<Entry>, parse_error_entries: List<Entry>, syms: Set<String>, assigns: Set<String>) {
            this.entries = lines
        ...
        fun recalculate(mem: MutableMap<String, Int>, assignment_post_processor: (Entry, Map<String, Int>) -> Int): Map<String, Int> {
    ...
    class MathExpr {
        ...
        enum class ExprType { VOID, CONST, VARREF, FUNC, NEG, NOT, MPY, DIV, MOD, ADD, SUB, AND, OR, XOR, NEQ, EQ, LE, GE, LT, GT, ASSIGN }
        ...
        abstract class ExprOp(type: ExprType, args: List<Expr>) : Expr(type, type.toString()) {
            ...
            override fun toString(): String = name + "{" + arguments.joinToString(",") { it.toString() } + "}"
        }
        ...
        class ExprFunc(
            ...
            init { if (nargs >= 0 && nargs != args.size) throw RuntimeException("invalid_number_of_arguments") }
            @Suppress("UNCHECKED_CAST")
            override fun calc(mem: Map<String, Int>): Int = func(arguments.toTypedArray(), mem as MutableMap<String, Int>)
            fun nargs(): Int = num_arguments
            override fun toString(): String = "FN{'$name'(${arguments.joinToString(",") { it.toString() }})}"
        }
        ...
        class ExprNot(args: List<Expr>) : ExprOp(ExprType.NOT, args) {
            override fun calc(mem: Map<String, Int>): Int = if (arguments[0].calc(mem) == 0) bool_true() else bool_false()
        }
        class ExprMpy(args: List<Expr>) : ExprOp(ExprType.MPY, args) {
            override fun calc(mem: Map<String, Int>): Int = arguments[0].calc(mem) * arguments[1].calc(mem)
        }
        class ExprDiv(args: List<Expr>) : ExprOp(ExprType.DIV, args) {
            override fun calc(mem: Map<String, Int>): Int { val b = arguments[1].calc(mem); return if (b <= 0) 0 else arguments[0].calc(mem) / b }
        }
        ...
        class ExprAnd(args: List<Expr>) : ExprOp(ExprType.AND, args) {
            override fun calc(mem: Map<String, Int>): Int = if (arguments[0].calc(mem) > 0 && arguments[1].calc(mem) > 0) bool_true() else bool_false()
        }
        class ExprOr(args: List<Expr>) : ExprOp(ExprType.OR, args) {
            override fun calc(mem: Map<String, Int>): Int = if (arguments[0].calc(mem) > 0 || arguments[1].calc(mem) > 0) bool_true() else bool_false()
        }
        class ExprXor(args: List<Expr>) : ExprOp(ExprType.XOR, args) {
            override fun calc(mem: Map<String, Int>): Int = if ((arguments[0].calc(mem) > 0) xor (arguments[1].calc(mem) > 0)) bool_true() else bool_false()
        }
        class ExprNeq(args: List<Expr>) : ExprOp(ExprType.NEQ, args) {
            override fun calc(mem: Map<String, Int>): Int = if (arguments[0].calc(mem) != arguments[1].calc(mem)) bool_true() else bool_false()
        }
        ...
        class ExprFuncDef(val name: String, val nargs: Int, val func: (Array<Expr>, MutableMap<String, Int>) -> Int)

        class ParsedLine private constructor(lineStr: String, defaultAssignmentVariable: String, funcDefs: Collection<ExprFuncDef>) {
            ...
            override fun toString(): String =
                "ParsedLine{" +
                " line:\"" + line.replace("\"", "\\\"") + "\"," +
                (if (error.isEmpty()) "" else " error:\"" + error.replace("\"", "\\\"") + " @pos=$pe\",") +
                " sym:\"" + symbols.joinToString(",") + "\"," +
                " expr:\"$expression\" " +
                "}"

            companion object {
                @JvmStatic fun of(line: String): ParsedLine = of(line, "")
                @JvmStatic fun of(line: String, defaultAssignmentVariable: String): ParsedLine = of(line, defaultAssignmentVariable, emptyList())
                @JvmStatic fun of(line: String, defaultAssignmentVariable: String, functions: Collection<ExprFuncDef>): ParsedLine =
                    ParsedLine(line, defaultAssignmentVariable, functions)
            }
            ...
            private fun adv() {
                if (++pe >= line.length) {
                    ...
                    val ci = line[pe].code
                    if (ci > 127) throw RuntimeException("invalid_character")
                    else c = line[pe].lowercaseChar()
                }
            }
            ...
            private fun adv(match: String): Boolean {
                while (c == ' ' || c == '\t' || c == '#') adv()
                if (!line.regionMatches(ignoreCase = true, thisOffset = pe, other = match, otherOffset = 0, length = match.length)) return false
                pe += match.length - 1
                adv()
                return true
            }
            ...
            private fun expr_assign(default_assignment_variable: String): Expr {
                ...
                if (line.matches(Regex("^[\\s]*[a-zA-Z][\\w.]*[\\s]*[=][^=].*"))) {
                    ...
                    if (functions.containsKey(ref.lowercase())) throw RuntimeException("symbol_readonly")
                }
                symbols.add(ref)
            ...
            private fun expr_or(): Expr {
                var x = expr_xor()
                while (true) {
                    x = if (adv("or")) ExprOr(listOf(x, expr_xor()))
                    else if (adv("||")) ExprOr(listOf(x, expr_xor()))
                    else if (adv('|')) ExprOr(listOf(x, expr_xor()))
                    else return x
                }
            }
            ...
            private fun expr_xor(): Expr {
                var x = expr_and()
                while (true) {
                    x = if (adv("xor")) ExprXor(listOf(x, expr_and()))
                    else if (adv('^')) ExprXor(listOf(x, expr_and()))
                    else return x
                }
            }
            ...
            private fun expr_and(): Expr {
                var x = expr_rel()
                while (true) {
                    x = if (adv("and")) ExprAnd(listOf(x, expr_rel()))
                    else if (adv("&&")) ExprAnd(listOf(x, expr_rel()))
                    else if (adv('&')) ExprAnd(listOf(x, expr_rel()))
                    else return x
                }
            }
            ...
            private fun expr_rel(): Expr {
                ...
                while (true) {
                    ...
                    if (adv('<'))  x = ExprLt(listOf(x, arith_add()))
                    else return x
                }
            }
            ...
            private fun arith_add(): Expr {
                var x = arith_mpy()
                while (true) {
                    x = if (adv('+')) ExprAdd(listOf(x, arith_mpy()))
                    else if (adv('-')) ExprSub(listOf(x, arith_mpy()))
                    else return x
                }
            }
            ...
            private fun arith_mpy(): Expr {
                var x = arith_fact()
                while (true) {
                    x = if (adv('*')) ExprMpy(listOf(x, arith_fact()))
                    else if (adv('/')) ExprDiv(listOf(x, arith_fact()))
                    else if (adv('%')) ExprMod(listOf(x, arith_fact()))
                    else return x
                }
            }
                ...
                if (adv('(')) {
                    ...
                    if (sym == "not") {
                        ...
                        if (!adv(')')) {
                            ...
                            if (!adv(')')) throw RuntimeException("missing_closing_function_parenthesis")
                        }
                        val fn = functions[sym] ?: throw RuntimeException("unknown_function")
                        return ExprFunc(fn.name, fn.nargs, fn.func, args)
                    } else {
                        if (functions.containsKey(sym)) throw RuntimeException("missing_function_arguments")
                        symbols.add(sym)
