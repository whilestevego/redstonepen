...
object ModRenderers {
    ...
    class TrackTer(private val renderer_: BlockEntityRendererProvider.Context?) : BlockEntityRenderer<TrackBlockEntity> {

        companion object {
            private val model_rls  = arrayOfNulls<ResourceLocation>(RedstoneTrackDefs.STATE_FLAG_WIR_COUNT)
            private val modelm_rls = arrayOfNulls<ResourceLocation>(RedstoneTrackDefs.STATE_FLAG_CON_COUNT)
            private val modelc_rls = arrayOfNulls<ResourceLocation>(RedstoneTrackDefs.STATE_FLAG_CON_COUNT)
            private val power_rgb  = ArrayList<Vec3>()

            @JvmStatic fun registerModels(): List<ResourceLocation> {
                val resources_to_register = ArrayList<ResourceLocation>()
                RedstoneTrackDefs.models.STATE_WIRE_MAPPING.forEach { (key, value) ->
                    val mrl = ResourceLocation.tryBuild(ModConstants.MODID, value)!!.withPrefix("item/")
        ...
        override fun render(te: TrackBlockEntity, unused1: Float, mxs: PoseStack, buf: MultiBufferSource, combinedLightIn: Int, combinedOverlayIn: Int) {
            ...
            try {
                ...
                val vxb: VertexConsumer = buf.getBuffer(ItemBlockRenderTypes.getRenderType(block_state, false))
                ...
                run {
                    ...
                    for (i in 0 until wirfc) {
                        if ((wirfl.toLong() and flag) != 0L) {
                            val rgb = getPowerRGB(te.getSidePower(RedstoneTrackDefs.connections.CONNECTION_BIT_ORDER[i / 4]))
                            val model = PlatformServices.getRendering().getBakedModel(Minecraft.getInstance().modelManager, model_rls[i]!!)
                            Minecraft.getInstance().blockRenderer.modelRenderer.renderModel(
                                mxs.last(), vxb, null, model,
                                rgb.x().toFloat(), rgb.y().toFloat(), rgb.z().toFloat(),
                                combinedLightIn, overlay
                            )
                        }
                        flag = flag shl 1
                    }
                ...
                run {
                    ...
                    for (i in 0 until confc) {
                        if ((wirfl.toLong() and wir) != 0L || (confl.toLong() and con) != 0L) {
                            val rgb = getPowerRGB(te.getSidePower(RedstoneTrackDefs.connections.CONNECTION_BIT_ORDER[i]))
                            val model = if ((confl.toLong() and con) == 0L)
                                PlatformServices.getRendering().getBakedModel(Minecraft.getInstance().modelManager, modelm_rls[i]!!)
                            else
                                PlatformServices.getRendering().getBakedModel(Minecraft.getInstance().modelManager, modelc_rls[i]!!)
                            Minecraft.getInstance().blockRenderer.modelRenderer.renderModel(
                                mxs.last(), vxb, null, model,
                                rgb.x().toFloat(), rgb.y().toFloat(), rgb.z().toFloat(),
                                combinedLightIn, overlay
                ...
                if (!broken_entities_.containsKey(te)) {
                    Auxiliaries.logError("TER render error for track at ${te.blockPos}, exception: ${e.message}")
