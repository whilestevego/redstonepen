...
class TooltipDisplay {
    ...
    class TipRange(x: Int, y: Int, w: Int, h: Int, text: Supplier<Component?>) {
        ...
        constructor(x: Int, y: Int, w: Int, h: Int, text: Component) : this(x, y, w, h, Supplier { text })
    }
    ...
    fun init(ranges: List<TipRange>): TooltipDisplay = init(ranges, defaultDelay, defaultMaxDeviation)
    fun init(vararg ranges: TipRange): TooltipDisplay = init(ranges.toList(), defaultDelay, defaultMaxDeviation)

    fun delay(ms: Int): TooltipDisplay { delay = if (ms <= 0) defaultDelay else ms.toLong(); return this }

    fun resetTimer() { t = System.currentTimeMillis() }

    fun <T : AbstractContainerMenu> render(gg: GuiGraphics, gui: AbstractContainerScreen<T>, x: Int, y: Int): Boolean {
        ...
        if (Math.abs(x - xLast) > maxDeviation || Math.abs(y - yLast) > maxDeviation) {
            ...
            return false
        } else if (ranges.stream().noneMatch { tip ->
            ...
            try {
                ...
                Auxiliaries.logError("Tooltip rendering disabled due to exception: '${ex.message}'")
    ...
    companion object {
        ...
        @JvmStatic fun config(delay: Int, maxDeviation: Int) {
