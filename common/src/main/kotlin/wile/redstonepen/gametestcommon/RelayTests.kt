...
object RelayTests {
    ...
    fun rightFacingIsRotationPlusOneRelativeToFront(helper: GameTestHelper) {
        ...
        for (face in Direction.values()) {
            for (r in 0..3) {
                ...
                if (actual != expected) {
                    helper.fail(
                        "getRightFacing FACING=$face ROTATION=$r: expected $expected got $actual"
                    )
                }
            }
        }
    ...
    fun backFacingIsRotationPlusTwoRelativeToFront(helper: GameTestHelper) {
        ...
        for (face in Direction.values()) {
            for (r in 0..3) {
                ...
                if (actual != expected) {
                    helper.fail(
                        "getBackFacing FACING=$face ROTATION=$r: expected $expected got $actual"
                    )
                }
            }
        }
    ...
    fun leftFacingIsRotationPlusThreeRelativeToFront(helper: GameTestHelper) {
        ...
        for (face in Direction.values()) {
            for (r in 0..3) {
                ...
                if (actual != expected) {
                    helper.fail(
                        "getLeftFacing FACING=$face ROTATION=$r: expected $expected got $actual"
                    )
                }
            }
        }
    ...
    fun allSixMappedDirectionsAreDistinctForEveryState(helper: GameTestHelper) {
        ...
        for (face in Direction.values()) {
            for (r in 0..3) {
                ...
                if (dirs.size != 6) {
                    helper.fail(
                        "FACING=$face ROTATION=$r: expected 6 distinct facing directions, got ${dirs.size}"
                    )
                }
            }
        }
        ...
        for (face in Direction.values()) {
            for (r in 0..3) {
                ...
                for (worldSide in Direction.values()) {
                    ...
                    if (back != worldSide) {
                        helper.fail(
                            "FACING=$face ROTATION=$r side=$worldSide: reverse(forward(side)) != side, got $back"
                        )
                    }
                }
            }
    ...
    fun getStateForPlacementReturnsNullWhenNoSurface(helper: GameTestHelper) {
        ...
        if (s != null) {
            helper.fail("expected null from getStateForPlacement when surface is not sturdy")
        }
        helper.succeed()
    }
    ...
    fun bridgeRelayGetInputPowerFromRedstoneWire(helper: GameTestHelper) {
        ...
        val sig =
            helper
                ...
                .getSignal(helper.getLevel(), helper.absolutePos(RELAY_POS), Direction.EAST)
    ...
    fun bridgeRelayGetSignalFromLeftWireConnection(helper: GameTestHelper) {
        ...
        val sig =
            helper
                .getBlockState(RELAY_POS)
                .getSignal(helper.getLevel(), helper.absolutePos(RELAY_POS), Direction.NORTH)
    ...
    private fun relayPlacementState(
    ...
    private fun assertRelayState(
        ...
        helper.assertBlockState(
            ...
            {
                it.`is`(relayBlock) &&
                    it.getValue(CircuitComponents.DirectedComponentBlock.POWERED) == powered &&
                    it.getValue(CircuitComponents.DirectedComponentBlock.STATE) == state
            },
            { message },
        )
    }
