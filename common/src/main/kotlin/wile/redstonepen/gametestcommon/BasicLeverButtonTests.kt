...
object BasicLeverButtonTests {
    ...
    fun leverUseWithoutItemTogglesPoweredFalseToTrue(helper: GameTestHelper) {
        ...
        if (before.getValue(BlockStateProperties.POWERED)) {
            helper.fail("expected initial powered=false")
        }
        ...
        helper.runAfterDelay(2) {
            ...
            if (!after.getValue(BlockStateProperties.POWERED)) {
                helper.fail("expected powered after lever toggle")
            }
            helper.succeed()
        }
    }
    ...
    fun gaugeReadsSignalFromAdjacentRedstoneBlock(helper: GameTestHelper) {
        ...
        helper.succeedWhen {
            val power = helper.getBlockState(POS).getValue(BlockStateProperties.POWER)
            if (power <= 0) {
                helper.fail("expected gauge power>0 adjacent to redstone block, got $power")
            }
        }
    }
