...
object ControlBoxTests {
    ...
    @JvmStatic fun controlBoxEvaluatesConstantProgram(helper: GameTestHelper) {
        val controlBox = placeControlBox(helper)
        controlBox.setCode("b=7")
        controlBox.setEnabled(true)
        controlBox.tick()
        helper.succeedWhen {
            val te = getControlBox(helper) ?: throw IllegalStateException("expected control box block entity to exist")
            if (!te.getEnabled()) throw IllegalStateException("expected control box to be enabled")
            if ("b=7" != te.getCode()) throw IllegalStateException("expected control box code to remain applied")
            val output = te.writenbt(helper.level.registryAccess(), CompoundTag(), false).getCompound("logic").getInt("output")
            if ((output shr (4 * Direction.EAST.ordinal)) and 0xf != 7)
                throw IllegalStateException("expected control box to compute a constant east-side output of 7")
        }
    }

    @JvmStatic fun controlBoxRejectsInvalidProgram(helper: GameTestHelper) {
        val controlBox = placeControlBox(helper)
        ...
        helper.succeedWhen {
            ...
            if (hooks.errors().isEmpty())
                throw IllegalStateException("expected invalid control box code to expose parse errors")
        }
    }

    @JvmStatic fun writenbtRoundTripsCodeAndSymbols(helper: GameTestHelper) {
        ...
        if ("b=5" != nbt.getCompound("logic").getString("code")) helper.fail("expected code persisted")
        helper.succeed()
    }

    @JvmStatic fun readnbtRestoresCodeAndOutputData(helper: GameTestHelper) {
        val te = placeControlBox(helper)
    ...
    @JvmStatic fun readnbtPreservesSymbolMap(helper: GameTestHelper) {
        val te = placeControlBox(helper)
    ...
    @JvmStatic fun initiallyDisabled(helper: GameTestHelper) {
        val te = placeControlBox(helper)
        if (te.getEnabled()) helper.fail("freshly placed control box must start disabled")
        helper.succeed()
    }

    @JvmStatic fun setEnabledTrueFlipsState(helper: GameTestHelper) {
        val te = placeControlBox(helper)
        te.setEnabled(true)
    ...
    @JvmStatic fun setEnabledIdempotent(helper: GameTestHelper) {
        val te = placeControlBox(helper)
        te.setEnabled(false)
        val te2 = getControlBox(helper)
    ...
    @JvmStatic fun tickOnDisabledBoxClearsOutput(helper: GameTestHelper) {
        val te = placeControlBox(helper)
        te.setCode("b=7")
        te.tick()
        val output = te.writenbt(helper.level.registryAccess(), CompoundTag(), false).getCompound("logic").getInt("output")
    ...
    @JvmStatic fun tickWithInvalidCodeDoesNotThrow(helper: GameTestHelper) {
        val te = placeControlBox(helper)
        te.setCode("b=d.bad")
        te.setEnabled(true)
        val te2 = getControlBox(helper)
    ...
    @JvmStatic fun traceToggleFlipsFlag(helper: GameTestHelper) {
        val te = placeControlBox(helper)
    ...
    @JvmStatic fun getNameFallsBackToBlockTranslationKey(helper: GameTestHelper) {
        val te = placeControlBox(helper)
    ...
    @JvmStatic fun setCustomNameStoresAndReturnsIt(helper: GameTestHelper) {
        val te = placeControlBox(helper)
    ...
    @JvmStatic fun onServerPacketReceivedAppliesCodeFromNbt(helper: GameTestHelper) {
        val te = placeControlBox(helper)
        val logic = CompoundTag()
    ...
    @JvmStatic fun getSignalReturnsComputedOutput(helper: GameTestHelper) {
        val te = placeControlBox(helper)
        te.setCode("b=7")
        te.setEnabled(true)
        te.tick()
        val block = Registries.getBlock("control_box")!! as ControlBoxBlock
        val signal = block.getSignal(helper.getBlockState(CONTROL_BOX_POS), helper.level, helper.absolutePos(CONTROL_BOX_POS), Direction.WEST)
        if (signal != 7) helper.fail("expected getSignal=7 queried from WEST, got $signal")
        helper.succeed()
    }

    @JvmStatic fun getDirectSignalMatchesGetSignal(helper: GameTestHelper) {
        val te = placeControlBox(helper)
        te.setCode("b=5")
        te.setEnabled(true)
        te.tick()
    ...
    @JvmStatic fun dropListWithCodeSavesNbt(helper: GameTestHelper) {
        val te = placeControlBox(helper)
        te.setCode("b=3")
        val block = Registries.getBlock("control_box")!! as ControlBoxBlock
    ...
    @JvmStatic fun dropListWithNoCodeHasNoNbt(helper: GameTestHelper) {
        val te = placeControlBox(helper)
        val block = Registries.getBlock("control_box")!! as ControlBoxBlock
        val drops = block.dropList(helper.getBlockState(CONTROL_BOX_POS), helper.level, te, false)
    ...
    @JvmStatic fun setPlacedByWithNbtRestoresCode(helper: GameTestHelper) {
        val te = placeControlBox(helper)
        te.setCode("b=9")
        ...
        val block = Registries.getBlock("control_box")!! as ControlBoxBlock
        block.setPlacedBy(helper.level, helper.absolutePos(CONTROL_BOX_POS), helper.getBlockState(CONTROL_BOX_POS), null, stack)
        val te2 = getControlBox(helper)
        if (te2 == null) { helper.fail("expected BE after setPlacedBy"); return }
        if ("b=9" != te2.getCode()) helper.fail("expected code restored by setPlacedBy, got: ${te2.getCode()}")
        helper.succeed()
    }

    @JvmStatic fun setPlacedByWithEmptyNbtIsNoOp(helper: GameTestHelper) {
    ...
    @JvmStatic fun updateWithNullFromPosResetsTickTimer(helper: GameTestHelper) {
        placeControlBox(helper)
        val block = Registries.getBlock("control_box")!! as ControlBoxBlock
        val absPos = helper.absolutePos(CONTROL_BOX_POS)
    ...
    @JvmStatic fun updateWithNeighborPosTriggersSideScan(helper: GameTestHelper) {
        placeControlBox(helper)
        val block = Registries.getBlock("control_box")!! as ControlBoxBlock
        val absPos = helper.absolutePos(CONTROL_BOX_POS)
        val result = block.update(helper.getBlockState(CONTROL_BOX_POS), helper.level, absPos, absPos.east())
        if (result == null) helper.fail("update must return non-null state")
        helper.succeed()
    }

    @JvmStatic fun isBlockEntityTickingAlwaysTrue(helper: GameTestHelper) {
        placeControlBox(helper)
        val block = Registries.getBlock("control_box")!! as ControlBoxBlock
    ...
    @JvmStatic fun defsDefaultConstructorIsCallable(helper: GameTestHelper) {
        Defs()
        if (Defs.PORT_NAMES.size != 6) helper.fail("PORT_NAMES must have 6 entries")
        helper.succeed()
    }

    @JvmStatic fun getDisplayNameReturnsNonNull(helper: GameTestHelper) {
        val te = placeControlBox(helper)
        if (te.displayName == null) helper.fail("getDisplayName must not return null")
        helper.succeed()
    }

    @JvmStatic fun createMenuReturnsNonNull(helper: GameTestHelper) {
        val te = placeControlBox(helper)
    ...
    @JvmStatic fun tickWithTraceEnabledCoversTracePaths(helper: GameTestHelper) {
        val te = placeControlBox(helper)
        te.toggle_trace(null)
        te.setCode("b=3")
        te.setEnabled(true)
    ...
    @JvmStatic fun tickWithTickrateSymbolSetsInterval(helper: GameTestHelper) {
        val te = placeControlBox(helper)
        te.setCode("b=3\ntickrate=10")
        te.setEnabled(true)
        te.tick()
    ...
    @JvmStatic fun setEnabledFalseFromEnabledClearsSymbols(helper: GameTestHelper) {
        val te = placeControlBox(helper)
        te.setCode("b=7")
        te.setEnabled(true)
        te.tick()
    ...
    @JvmStatic fun setRcaPlayerUuidNonNullDoesNotThrow(helper: GameTestHelper) {
        val te = placeControlBox(helper)
        te.setRcaPlayerUUID(UUID.randomUUID())
        helper.succeed()
    }

    @JvmStatic fun signalUpdateRisingEdgeSetsIntredge(helper: GameTestHelper) {
        val te = placeControlBox(helper)
        te.setCode("b=g")
        te.setEnabled(true)
        te.tick()
    ...
    @JvmStatic fun signalUpdateFallingEdgeSetsIntfedge(helper: GameTestHelper) {
        val te = placeControlBox(helper)
        te.setCode("b=g")
        te.setEnabled(true)
        helper.setBlock(CONTROL_BOX_POS.west(), Blocks.REDSTONE_BLOCK)
    ...
    @JvmStatic fun useItemOnWithDebugStickTogglesTrace(helper: GameTestHelper) {
        placeControlBox(helper)
        val player = helper.makeMockPlayer(GameType.SURVIVAL)
        ...
        val result = helper.getBlockState(CONTROL_BOX_POS).useItemOn(debugStick, helper.level, player, InteractionHand.MAIN_HAND, hit)
        if (result == null) helper.fail("useItemOn must not return null")
        helper.succeed()
    }

    @JvmStatic fun controlBoxBuiltinFunctionsExercised(helper: GameTestHelper) {
        val te = placeControlBox(helper)
        val block = Registries.getBlock("control_box")!! as ControlBoxBlock
        val absPos = helper.absolutePos(CONTROL_BOX_POS)
        te.setEnabled(true)
        ...
        var sig = block.getSignal(helper.getBlockState(CONTROL_BOX_POS), helper.level, absPos, Direction.WEST)
        if (sig != 5) helper.fail("max(3,5) expected 5, got $sig")

        te.setCode("b=min(3,5)"); te.tick()
        sig = block.getSignal(helper.getBlockState(CONTROL_BOX_POS), helper.level, absPos, Direction.WEST)
    ...
    @JvmStatic fun controlBoxCounterFunctionsExercised(helper: GameTestHelper) {
        val te = placeControlBox(helper)
        ...
        te.tick()
        val block = Registries.getBlock("control_box")!! as ControlBoxBlock
        val sig = block.getSignal(helper.getBlockState(CONTROL_BOX_POS), helper.level, helper.absolutePos(CONTROL_BOX_POS), Direction.WEST)
        if (sig != 2) helper.fail("cnt3(1,10) after 2 ticks expected 2, got $sig")
        helper.succeed()
    }

    @JvmStatic fun controlBoxTimerFunctionsExercised(helper: GameTestHelper) {
        val te = placeControlBox(helper)
    ...
    @JvmStatic fun controlBoxRemainingTimerVariantsExercised(helper: GameTestHelper) {
        val te = placeControlBox(helper)
    ...
    @JvmStatic fun controlBoxTimerEdgeCasesDoNotThrow(helper: GameTestHelper) {
        val te = placeControlBox(helper)
    ...
    @JvmStatic fun controlBoxSetCodeSameCodeSkipsReparse(helper: GameTestHelper) {
        val te = placeControlBox(helper)
        te.setCode("b=7")
        te.setEnabled(true)
        te.tick()
    ...
    private fun placeControlBox(helper: GameTestHelper): ControlBoxBlockEntity {
        helper.setBlock(CONTROL_BOX_POS, Registries.getBlock("control_box")!!.defaultBlockState())
        return getControlBox(helper) ?: throw IllegalStateException("expected control box block entity to be created")
    }
