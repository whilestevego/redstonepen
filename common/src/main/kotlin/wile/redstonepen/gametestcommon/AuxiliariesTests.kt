...
object AuxiliariesTests {
    ...
    @JvmStatic fun particlesOnServerLevelDoesNotThrow(helper: GameTestHelper) {
        Auxiliaries.particles(helper.level, helper.absolutePos(POS), ParticleTypes.SMOKE)
        helper.succeed()
    }

    @JvmStatic fun particlesVec3OnServerLevelDoesNotThrow(helper: GameTestHelper) {
    ...
    @JvmStatic fun getFakePlayerOnServerLevelReturnsPresent(helper: GameTestHelper) {
        val result = Auxiliaries.getFakePlayer(helper.level)
        if (result == null) helper.fail("getFakePlayer must not return null Optional")
        helper.succeed()
    }

    @JvmStatic fun playerChatMessageWithMockPlayerDoesNotThrow(helper: GameTestHelper) {
        val player = helper.makeMockPlayer(GameType.SURVIVAL)
        Auxiliaries.playerChatMessage(player, "test.message")
        helper.succeed()
    }

    @JvmStatic fun serializeNonNullComponentReturnsNonEmpty(helper: GameTestHelper) {
        val serialized = Auxiliaries.serializeTextComponent(
            Component.literal("hello"), helper.level.registryAccess())
        if (serialized == null || serialized.isEmpty())
            helper.fail("serialize of non-null component must return non-empty string")
        helper.succeed()
    }

    @JvmStatic fun unserializeSimpleTextComponentReturnsComponent(helper: GameTestHelper) {
        val json = Auxiliaries.serializeTextComponent(
            Component.literal("hi"), helper.level.registryAccess())
