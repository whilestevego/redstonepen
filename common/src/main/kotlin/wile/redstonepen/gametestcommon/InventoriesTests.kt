...
object InventoriesTests {
    ...
    @JvmStatic fun inventoryRangeInsertBetweenMatchedSlots(helper: GameTestHelper) {
        ...
        if (!remaining.isEmpty)
            helper.fail("expected full insertion into slot 2 between two full match slots, got remaining=${remaining.count}")
        helper.succeed()
    }

    @JvmStatic fun inventoryRangeInsertBeforeAfterMatchedSlot(helper: GameTestHelper) {
        val container = SimpleContainer(5)
        container.setItem(1, ItemStack(Items.REDSTONE, 64))
        ...
        val remaining = ir.insert(ItemStack(Items.REDSTONE, 1))
        if (!remaining.isEmpty)
            helper.fail("expected insertion before match slot via before/after path, got remaining=${remaining.count}")
        helper.succeed()
    }

    @JvmStatic fun inventoryRangeInsertSimulateWithPartialStack(helper: GameTestHelper) {
        ...
        if (!remaining.isEmpty)
            helper.fail("simulate insert into partial stack must return empty, got ${remaining.count}")
        helper.succeed()
    }

    @JvmStatic fun inventoryRangeInsertSimulateNoRoomReturnsInput(helper: GameTestHelper) {
    ...
    @JvmStatic fun inventoryRangeExtractMultipleMatchingSlots(helper: GameTestHelper) {
        val container = SimpleContainer(3)
    ...
    @JvmStatic fun inventoryRangeExtractWithSimulateTrue(helper: GameTestHelper) {
        val container = SimpleContainer(2)
        ...
        if (container.getItem(0).count != 20)
            helper.fail("simulate extract must not modify the container, got ${container.getItem(0).count}")
        helper.succeed()
    }

    @JvmStatic fun inventoryRangeInsertFillupOnlyStopsAtOnlyFillup(helper: GameTestHelper) {
        val container = SimpleContainer(3)
