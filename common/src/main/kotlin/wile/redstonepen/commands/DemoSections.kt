...
object DemoSections {
    ...
    @JvmStatic fun runCircuits(level: Level, origin: BlockPos) {
        val contraptions: List<(Level, BlockPos) -> Unit> = listOf(
    ...
    @JvmStatic fun buildLeverDrivesLamp(level: Level, cell: BlockPos) {
        platform(level, cell)
        ...
        for (dz in 3 downTo 1) level.setBlock(cell.offset(2, 0, dz), Blocks.REDSTONE_WIRE.defaultBlockState(), FLAGS)
    ...
    @JvmStatic fun buildButtonDrivesPiston(level: Level, cell: BlockPos) {
        platform(level, cell)
        ...
        level.setBlock(cell.offset(2, 0, 4),
            button.defaultBlockState()
                .setValue(BlockStateProperties.ATTACH_FACE, AttachFace.FLOOR)
                .setValue(BlockStateProperties.HORIZONTAL_FACING, Direction.NORTH), FLAGS)
        for (dz in 3 downTo 2) level.setBlock(cell.offset(2, 0, dz), Blocks.REDSTONE_WIRE.defaultBlockState(), FLAGS)
    ...
    @JvmStatic fun buildInvertedRelayNotGate(level: Level, cell: BlockPos) {
        platform(level, cell)
    ...
    @JvmStatic fun buildBistableToggle(level: Level, cell: BlockPos) {
        platform(level, cell)
    ...
    @JvmStatic fun buildPulseRelayMonostable(level: Level, cell: BlockPos) {
        platform(level, cell)
    ...
    @JvmStatic fun buildRelayBuffer(level: Level, cell: BlockPos) {
        platform(level, cell)
    ...
    @JvmStatic fun buildBridgeRelayCrossover(level: Level, cell: BlockPos) {
        platform(level, cell)
    ...
    @JvmStatic fun buildControlBoxAndGate(level: Level, cell: BlockPos) {
        platform(level, cell)
    ...
    @JvmStatic fun buildGaugeReadout(level: Level, cell: BlockPos) {
        platform(level, cell)
    ...
    @JvmStatic fun buildPenTrackWallClimb(level: Level, cell: BlockPos) {
        platform(level, cell)
        ...
        placePenTrack(level, cell.offset(4, 0, 5),
            wireBit(Direction.DOWN, Direction.NORTH)
                or wireBit(Direction.DOWN, Direction.SOUTH)
                or wireBit(Direction.NORTH, Direction.DOWN)
                or wireBit(Direction.NORTH, Direction.UP))
        ...
        placePenTrack(level, cell.offset(4, 3, 4),
            wireBit(Direction.DOWN, Direction.SOUTH)
                or wireBit(Direction.DOWN, Direction.NORTH))
    ...
    @JvmStatic fun buildTrafficLight(level: Level, cell: BlockPos) {
        platform(level, cell)
    ...
    @JvmStatic fun buildPulseCounter(level: Level, cell: BlockPos) {
        platform(level, cell)
    ...
    @JvmStatic fun buildSrLatch(level: Level, cell: BlockPos) {
        platform(level, cell)
    ...
    @JvmStatic fun buildPwmDemo(level: Level, cell: BlockPos) {
        platform(level, cell)
    ...
    @JvmStatic fun buildStepSequencer(level: Level, cell: BlockPos) {
        platform(level, cell)
    ...
    @JvmStatic fun buildHoldTimer(level: Level, cell: BlockPos) {
        platform(level, cell)
    ...
    @JvmStatic fun refreshWireConnections(level: Level, cell: BlockPos) {
    ...
    private fun platform(level: Level, cell: BlockPos) {
        ...
        val air = Blocks.AIR.defaultBlockState()
        for (dx in 0 until CELL_SIZE) for (dz in 0 until CELL_SIZE) {
    ...
    private fun placeFloorRelay(level: Level, cell: BlockPos, relay: net.minecraft.world.level.block.Block) {
    ...
    private fun wireAndLamp(level: Level, cell: BlockPos) {
        for (dz in 2 downTo 1) level.setBlock(cell.offset(2, 0, dz), Blocks.REDSTONE_WIRE.defaultBlockState(), FLAGS)
        level.setBlock(cell.offset(2, 0, 0), Blocks.REDSTONE_LAMP.defaultBlockState(), FLAGS)
    }
    ...
    // ControlBox programs
    private val CONTROL_BOX_AND_PROGRAM = listOf(
        "# AND of south and west inputs",
        "b = if(y, if(g, 15, 0), 0)"
    ).joinToString("\n")

    private val TRAFFIC_LIGHT_PROGRAM = listOf(
    ...
    private val PULSE_COUNTER_PROGRAM = listOf(
    ...
    private val HOLD_TIMER_PROGRAM = listOf(
