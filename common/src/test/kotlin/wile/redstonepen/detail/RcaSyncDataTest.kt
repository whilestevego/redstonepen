...
class RcaSyncDataTest {
    @Test
    fun rcaDataWithZeroUUIDIsInvalid() {
        assertFalse(RcaData(UUID(0, 0)).isValid())
    }

    @Test
    fun rcaDataWithNonZeroUUIDIsValid() {
        assertTrue(RcaData(UUID.randomUUID()).isValid())
    }

    @Test
    fun rcaDataClientInputsRoundTrips() {
    ...
    @Test
    fun rcaDataServerOutputsRoundTrips() {
        val data = RcaData(UUID.randomUUID())
    ...
    @Test
    fun rcaDataToStringContainsHexClientInputs() {
        val data = RcaData(UUID.randomUUID())
    ...
    @Test
    fun rcaDataToStringContainsHexServerOutputs() {
        val data = RcaData(UUID.randomUUID())
    ...
    @Test
    fun commonRcaEmptyIsInvalid() {
        assertFalse(CommonRca.EMPTY.isValid())
    }

    @Test
    fun commonRcaOfPlayerNullReturnsEmpty() {
        assertSame(CommonRca.EMPTY, CommonRca.ofPlayer(null, false))
    }

    @Test
    fun commonRcaOfPlayerUnknownUUIDWithoutCreateReturnsEmpty() {
        assertSame(CommonRca.EMPTY, CommonRca.ofPlayer(UUID.randomUUID(), false))
    }

    @Test
    fun commonRcaOfPlayerCreatesEntryWhenAllowCreate() {
    ...
    @Test
    fun commonRcaOfPlayerReturnsSameInstanceOnSubsequentCalls() {
        val uid = UUID.randomUUID()
    ...
    @Test
    fun rcaDataToStringContainsUuid() {
        val uid = UUID.randomUUID()
        assertTrue(RcaData(uid).toString().contains(uid.toString()))
    }

    @Test
    fun rcaDataToStringShowsZeroClientOutputs() {
        val data = RcaData(UUID.randomUUID())
        assertTrue(data.toString().contains("co:0000000000000000"))
    }

    @Test
    fun commonRcaApplyRcaUpdateIgnoresNbtWithoutInputKey() {
        val nbt = CompoundTag()
        nbt.putLong("x", 42L)
    ...
    @Test
    fun commonRcaApplyRcaUpdateAppliesClientInputsAndWritesServerOutputs() {
        val uid = UUID.randomUUID()
