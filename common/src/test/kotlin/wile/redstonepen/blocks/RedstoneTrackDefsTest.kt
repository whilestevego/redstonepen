...
class RedstoneTrackDefsTest {
    ...
    @Test fun getWireBitYieldsKnownValues() {
    ...
    @Test fun getWireBitForUnknownPairReturnsZero() {
        assertEquals(0L, connections.getWireBit(Direction.DOWN, Direction.DOWN))
    }

    @Test fun getWireBitSideAndDirectionRoundTrip() {
    ...
    @Test fun getWireBitSideAndDirectionUnknownDefaultsDownDown() {
        val t = connections.getWireBitSideAndDirection(0xFFFFFFFFL)
        assertEquals(Direction.DOWN, t.a)
        assertEquals(Direction.DOWN, t.b)
    }

    @Test fun getVanillaWireConnectionDirectionsReturnsAllSetSides() {
    ...
    @Test fun getVanillaWireConnectionDirectionsEmptyForZeroMask() {
        assertTrue(connections.getVanillaWireConnectionDirections(0L).isEmpty())
    }

    @Test fun hasVanillaWireConnectionPerSide() {
    ...
    @Test fun hasBulkConnectionDetectsFaceBit() {
    ...
    @Test fun hasRedstoneConnectionAllSidesAtLeastOneBitTrue() {
    ...
    @Test fun hasRedstoneConnectionFalseForZero() {
        for (d in Direction.values()) {
            assertFalse(connections.hasRedstoneConnection(0L, d))
        }
    }

    @Test fun getWireElementsOnFaceCovers4BitsPerFace() {
        for (d in Direction.values()) {
    ...
    @Test fun getAllElementsOnFaceCoversWiresAndConnector() {
        for (d in Direction.values()) {
    ...
    @Test fun redstoneUpdateDirectionsContainsAllSixFaces() {
