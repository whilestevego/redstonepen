...
class ExtendedShapelessRecipeTest {
    ...
    private fun recipe(ingredientCount: Int, aspects: CompoundTag): ExtendedShapelessRecipe {
        ...
        return ExtendedShapelessRecipe("test_group", CraftingBookCategory.MISC,
            ItemStack(Items.REDSTONE), ingredients, aspects)
    }
    ...
    @Nested inner class Aspects {
        @Test fun isSpecialFalseForEmptyAspects() {
            assertFalse(recipe(1, CompoundTag()).isSpecial())
        }

        @Test fun isSpecialTrueWhenDynamicFlagSet() {
            val aspects = CompoundTag()
            aspects.putBoolean("dynamic", true)
            assertTrue(recipe(1, aspects).isSpecial())
        }

        @Test fun isSpecialTrueWhenToolRepairSet() {
            val aspects = CompoundTag()
            aspects.putInt("tool_repair", 100)
            assertTrue(recipe(1, aspects).isSpecial())
        }

        @Test fun isSpecialFalseWhenOnlyToolDamageSet() {
            val aspects = CompoundTag()
            aspects.putInt("tool_damage", 50)
            assertFalse(recipe(1, aspects).isSpecial())
        }

        @Test fun getAspectsReturnsCopyNotReference() {
            val aspects = CompoundTag()
            aspects.putBoolean("dynamic", true)
    ...
    @Nested inner class Dimensions {
        @Test fun canCraftInDimensionsTrueWhenGridFitsIngredients() {
        ...
        @Test fun canCraftInDimensionsFalseWhenGridTooSmall() {
            assertFalse(recipe(9, CompoundTag()).canCraftInDimensions(2, 4))
            assertFalse(recipe(2, CompoundTag()).canCraftInDimensions(1, 1))
        }
    }

    @Nested inner class Metadata {
        @Test fun getGroupReturnsConstructorArgument() {
            val ingredients = NonNullList.withSize(1, Ingredient.EMPTY)
            val r = ExtendedShapelessRecipe(
                "my_group", CraftingBookCategory.MISC, ItemStack(Items.REDSTONE),
                ingredients, CompoundTag())
            assertEquals("my_group", r.group)
        }

        @Test fun categoryReturnsConstructorArgument() {
            assertEquals(CraftingBookCategory.MISC, recipe(1, CompoundTag()).category())
        }

        @Test fun getSerializerNotNull() {
            assertNotNull(recipe(1, CompoundTag()).serializer)
        }

        @Test fun serializerExposesCodecs() {
            assertNotNull(ExtendedShapelessRecipe.SERIALIZER.codec())
            assertNotNull(ExtendedShapelessRecipe.SERIALIZER.streamCodec())
        }

        @Test fun getIngredientsReflectsConstructor() {
            val ingredients = NonNullList.withSize(2, Ingredient.EMPTY)
            val r = ExtendedShapelessRecipe(
                "g", CraftingBookCategory.MISC, ItemStack(Items.REDSTONE),
                ingredients, CompoundTag())
            assertEquals(2, r.ingredients.size)
        }
    }

    @Nested inner class ResultItem {
        @Test fun getResultItemReturnsConfiguredOutputForNonSpecial() {
            val out = recipe(1, CompoundTag()).getResultItem(null)
            assertEquals(Items.REDSTONE, out.item)
        }

        @Test fun getResultItemEmptyForSpecialDynamicRecipe() {
            val aspects = CompoundTag()
            aspects.putBoolean("dynamic", true)
            assertTrue(recipe(1, aspects).getResultItem(null).isEmpty)
        }

        @Test fun assembleReturnsResultStackForBasicRecipe() {
            val r = ExtendedShapelessRecipe(
                "g", CraftingBookCategory.MISC, ItemStack(Items.IRON_PICKAXE),
                NonNullList.withSize(1, Ingredient.EMPTY), CompoundTag())
        ...
        @Test fun assembleAppliesInitialDurability() {
            val aspects = CompoundTag()
        ...
        @Test fun assembleAppliesInitialDamage() {
            val aspects = CompoundTag()
        ...
        @Test fun assembleEmptyResultYieldsEmpty() {
            val r = ExtendedShapelessRecipe(
    ...
    @Nested inner class RemainingItems {
        @Test fun getRemainingItemsReturnsListOfSameSizeAsInput() {
            val r = recipe(1, CompoundTag())
            val inv = CraftingInput.of(1, 1, listOf(ItemStack(Items.STICK)))
            assertEquals(1, r.getRemainingItems(inv).size)
        }

        @Test fun getRemainingItemsRetainsToolWhenNotDamageable() {
            val aspects = CompoundTag()
        ...
        @Test fun getRemainingItemsDamagesDamageableTool() {
            val aspects = CompoundTag()
            aspects.putString("tool", "minecraft:iron_pickaxe")
        ...
        @Test fun getRemainingItemsConsumesToolWhenDamageExceedsMax() {
            val aspects = CompoundTag()
            aspects.putString("tool", "minecraft:iron_pickaxe")
    ...
    @Nested inner class RepairFlow {
        @Test fun toolRepairAspectMakesRecipeSpecialAndChangesResult() {
            val aspects = CompoundTag()
            aspects.putString("tool", "minecraft:iron_pickaxe")
            aspects.putInt("tool_repair", 100)
            val r = recipe(1, aspects)
        ...
        @Test fun repairAssembleReturnsHealedToolStack() {
        ...
        @Test fun repairGetRemainingItemsConsumesRepairItem() {
            val r = repairRecipe(50)
            val pickaxe = ItemStack(Items.IRON_PICKAXE)
            pickaxe.setDamageValue(120)
        ...
        @Test fun repairWithUndamagedToolReturnsEmptyAssemble() {
            val r = repairRecipe(50)
            val pickaxe = ItemStack(Items.IRON_PICKAXE)
        ...
        @Test fun repairOverRepairAspectAllowsZeroDamageInput() {
            val aspects = CompoundTag()
            aspects.putString("tool", "minecraft:iron_pickaxe")
        ...
        @Test fun repairRelativeDamagePathTriggers() {
            val aspects = CompoundTag()
            aspects.putString("tool", "minecraft:iron_pickaxe")
        ...
        @Test fun repairWithMissingToolReturnsEmpty() {
            val r = repairRecipe(50)
            val inv = CraftingInput.of(2, 1, listOf(
                ItemStack(Items.IRON_INGOT), ItemStack(Items.IRON_INGOT)))
            assertTrue(r.assemble(inv, null).isEmpty)
        }

        @Test fun repairWithNonDamageableToolReturnsEmpty() {
            val aspects = CompoundTag()
            aspects.putString("tool", "minecraft:stick")
            aspects.putInt("tool_repair", 50)
            val r = recipe(2, aspects)
            val inv = CraftingInput.of(2, 1, listOf(
                ItemStack(Items.STICK), ItemStack(Items.IRON_INGOT)))
            assertTrue(r.assemble(inv, null).isEmpty)
        }
    }

    @Nested inner class Matches {
        @Test fun matchesEmptyInputForRecipeWithIngredients() {
            val r = recipe(1, CompoundTag())
