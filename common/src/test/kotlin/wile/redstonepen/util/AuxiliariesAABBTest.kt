...
class AuxiliariesAABBTest {
    ...
    @Nested inner class PixeledAABB {
        @Test fun pixeledAABBDividesAllCoordinatesBySixteen() {
            assertAABB(AABB(0.0, 0.0, 0.0, 1.0, 1.0, 1.0), Auxiliaries.getPixeledAABB(0.0, 0.0, 0.0, 16.0, 16.0, 16.0))
        }

        @Test fun pixeledAABBPreservesAsymmetricBounds() {
            assertAABB(AABB(0.25, 0.5, 0.0, 0.75, 1.0, 0.25), Auxiliaries.getPixeledAABB(4.0, 8.0, 0.0, 12.0, 16.0, 4.0))
        }

        @Test fun getPixeledAABBPartialPixels() {
    ...
    @Nested inner class RotatedAABB {
        ...
        @Test fun getRotatedAABBArrayPreservesLength() {
        ...
        @Test fun getRotatedAABBArrayWithEmptyInput() {
            assertEquals(0, Auxiliaries.getRotatedAABB(emptyArray<AABB>(), Direction.NORTH).size)
        }
    }

    @Nested inner class RotatedAABBHorizontal {
        @Test fun getRotatedAABBHorizontalKeepsYAxis() {
        ...
        @Test fun getRotatedAABBHorizontalDownAndUpAreIdentity() {
            val bb = AABB(0.1, 0.25, 0.3, 0.7, 0.85, 0.9)
        ...
        @Test fun getRotatedAABBHorizontalAllSixFacesProduceValidBoxes() {
            val bb = AABB(0.1, 0.2, 0.3, 0.6, 0.7, 0.9)
            for (d in Direction.values()) {
        ...
        @Test fun getRotatedAABBArrayHorizontalPreservesLength() {
    ...
    @Nested inner class YRotatedAABB {
        @Test fun yRotatedAABBZeroStepsIsIdentity() {
            val bb = AABB(0.1, 0.2, 0.3, 0.6, 0.7, 0.8)
            assertAABB(bb, Auxiliaries.getYRotatedAABB(bb, 0))
        }

        @Test fun yRotatedAABBFourStepsIsIdentity() {
            val bb = AABB(0.1, 0.2, 0.3, 0.6, 0.7, 0.8)
            assertAABB(bb, Auxiliaries.getYRotatedAABB(bb, 4))
        }

        @Test fun yRotatedAABBTwoStepsIs180Degrees() {
            val bb = AABB(0.1, 0.2, 0.3, 0.6, 0.7, 0.8)
            assertAABB(AABB(0.4, 0.2, 0.2, 0.9, 0.7, 0.7), Auxiliaries.getYRotatedAABB(bb, 2))
        }

        @Test fun getYRotatedAABBHandlesNegativeAndModularSteps() {
            val bb = AABB(0.1, 0.25, 0.3, 0.7, 0.85, 0.9)
        ...
        @Test fun getYRotatedAABBAllFourQuartersProduceValidBoxes() {
            val bb = AABB(0.1, 0.2, 0.3, 0.6, 0.7, 0.9)
            val eps = 1e-9
            for (q in 0 until 4) {
    ...
    @Nested inner class UnionShape {
        ...
        @Test fun getUnionShapeArrayOfArraysHandlesEmptyInput() {
